...
- `#!python "postproc[<name>]["raw"]"`: a `Raw` `N x 2` array of `(x, density)` for plotting.
- `#!python "postproc[<name>]["summary"]"`: a `Summary` table of the trace's moments and quantiles (`count`, `mean`, `std`, `min`, `q25`, `median`, `q75`, `max`).
