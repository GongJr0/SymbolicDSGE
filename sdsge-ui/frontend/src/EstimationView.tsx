            ...
            content: (
                <div className="estimation-parameter-list">
                    {parameters.map((parameter) => (
                        <button
                            key={parameter.name}
                            className={`estimation-parameter-card ${parameter.estimate ? "estimated" : ""
                ...
                <section className="estimation-result">
                    ...
                    {covStatus !== 0 && (
                        <span className="muted">
                            {`No standard errors: ${COV_STATUS_REASONS[covStatus] ?? `covariance status ${covStatus}`
                                }.`}
                        </span>
                    )}
...
function ProposalCovariance({
    ...
    if (value === null) {
        return (
            <span className="muted">
                {`Proposal covariance: derived from the Hessian at ${computeMap ? "the MAP estimate" : "the starting values"
                    }.`}
            </span>
        );
    }
