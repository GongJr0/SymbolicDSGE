...
def _lower_filter_step(
    ...
    if mode == "extended":
        ...
        bindings = _filter_bindings(
            (interface.A, interface.B, params, interface.Q, interface.R),
            y_binding,
            (x0, interface.P0),
        )
        return (
    ...
    if mode == "unscented":
        ...
        y_offset = sum(values.size for values in before_y)
        y_binding = _filter_y_binding(
            source_layout,
            T,
            source_columns,
        ...
        bindings = _filter_bindings(before_y, y_binding, (z0, interface.P0))
        return (
            filter_unscented_step(
                step.name,
                measurement_addr,
