...
class SymbolicRegressorBackend:
    ...
    def _get_sp_from_template(self, expr_block: pd.Series) -> pd.Series:
        ...
        # Parse Expr with primitive operators mapped (This replaces function calls with user defined bodies)
        primitives: dict[str, Callable[..., sp.Expr]] = self.parametrizer.primitive_ops
        var_names = self.parametrizer.variable_names

        local_vars = (
            {var: sp.Symbol(var) for var in var_names} if var_names is not None else {}
        )
        locals = {**primitives, **local_vars}

        expr_subbed = sp.parse_expr(expr_str, local_dict=locals)
        subbed_str = str(expr_subbed)

        out = expr_block.copy()
        out["equation"] = subbed_str
    ...
    def _convert_and_handle_constants(self, equations: pd.DataFrame) -> pd.DataFrame:
        ...
        converted["sympy_format"] = list(handled)  # pyright: ignore
