% make_rbc_occbin_goldens.m
  % Run after:
  %   dynare rbc_occbin.mod noclearall
  %
  % Prints every golden to stdout at %.17g and writes no files. Transcribe the
  % session log into tests/_oracles/dynare_rbc_occbin.py.
  %
  ...
  %
  % Length comes from the schedule, NOT from simul_periods: the last declared
  % event is at 169, and Dynare simulates the whole sequence regardless of a
  % shorter simul_periods. Shortening a run means editing the shocks block.
  T_shock = 169;
  shock_rebuilt = zeros(T_shock, 1);
  shock_rebuilt(1:9) = -0.0001;
  shock_rebuilt(10) = -0.01;
  shock_rebuilt(50) = -0.02;
  shock_rebuilt(90) = 0.01;
  shock_rebuilt(130) = 0.02;
  shock_rebuilt(131:169) = 0;

  fprintf('### shock_rebuilt (%d x 1, epsilon)\n', T_shock);
  for i = 1:T_shock
  ...
  if isfield(oo_, 'occbin') && isfield(oo_.occbin, 'simul')
      ...
      % simulation with the constraints ignored, so it checks our reference
      ...
      if isfield(simul, 'regime_history')
          ...
          % Run-length encoded, one date per line. regimeN holds the 0/1 values
          % and regimestartN the offsets they take effect at, counted from the
          ...
          % whole.
          %
          % N follows the occbin_constraints declaration order, so regime1 is
          % IRR and regime2 is INEG. That is NOT the order the occbin_*_bind
          % parameters appear in M_.param_names, which follows first use in the
          % model block and puts INEG first.
          fprintf(['### regime_history ' ...
                   '(t | regime1 | regimestart1 | regime2 | regimestart2)\n']);
          has2 = isfield(rh, 'regime2');
          for tt = 1:numel(rh)
              fprintf('%d|', tt);
              printint(rh(tt).regime1(:)');
              fprintf('|');
              printint(rh(tt).regimestart1(:)');
              if has2
                  fprintf('|');
                  printint(rh(tt).regime2(:)');
                  fprintf('|');
                  printint(rh(tt).regimestart2(:)');
              end
              fprintf('\n');
          end
          ...
          fprintf('### realized_regimes (%d x %d, columns = [IRR, INEG])\n', ...
                  numel(rh), 1 + has2);
          for tt = 1:numel(rh)
              row = rh(tt).regime1(1);
              if has2
                  row = [row, rh(tt).regime2(1)];
              end
              printint(row);
              fprintf('\n');
          end
