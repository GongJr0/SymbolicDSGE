 ...
 *    parameter that makes sure that in steady-state y=1.
...
/*
 * FIXTURE NOTE. This is the parity fixture for the one-constraint OccBin
 * driver, so it is the upstream mod file reduced to the run the golden
 * generator reads, and to numbers our own config can state:
 *
 *  - Every number the equations reference is a literal parameter here, so the
 *    two configs share it rather than each deriving it. PSI is no longer
 *    calibrated inside steady_state_model, and steady_state(r) / steady_state(y)
 *    / steady_state(pie) are R_SS / Y_SS / PI. All four carry the value the
 *    upstream steady_state_model computes, so `steady` still checks them: a
 *    wrong literal makes the Euler equation and the policy rule disagree on r
 *    and the residual check fails.
 *  - The surprise block is padded with zeros out to period 30 so that Dynare
 *    simulates and records a regime for every date it reports, rather than the
 *    6 the shock alone would give. Zero shocks do not move the path, so the
 *    replication is unaffected.
 *  - Only the positive discount-factor shock is run, the column of figure 5
 *    where the constraint binds. The truncation to periods 6:15, the graphs and
 *    the reporting fprintfs are dropped: the generator reads oo_ whole.
 */
...
parameters
  ...
  PSI        ${\psi}$               (long_name='utility weight leisure')
  ZLB        ${ZLB}$                (long_name='value of zero lower bound')
  R_SS       ${\bar R}$             (long_name='steady-state nominal interest rate')
...
PHI     = 1;
ZLB     = 1;
% PSI is w/(l^PHI*c) at the steady state, the value that puts y at Y_SS.
...
r = R_SS*(pie/PI)^PHI_PI*(y/Y_SS)^PHI_Y;
...
% increase in the discount factor, the column of Fig. 5 where the ZLB binds.
% The trailing zeros carry the shock array out to the reported horizon.
shocks(surprise,overwrite);
var epsi; periods 6, 7:30; values @{SHOCKSIZE}, 0;
end;
